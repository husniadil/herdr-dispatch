...
| §3.7 | An operator verb performed by a principal other than the operator records the CALLING principal as the event's actor, never `human`, and the event is MARKED as an operator verb performed by someone other than the operator | `parked.resolve` is declared the operator's authority and therefore advisory (`internal/verbs/verbs.go`), so §3.7 reaches this plugin. Neither half of its trail duty is implemented. The resolved event's actor is always this daemon's own principal (`internal/loop/events.go`), because every event here is written as the daemon acting; who resolved the row is carried only as `resolved_by` inside `detail` (`internal/loop/loop.go`), and there is no operator-verb mark at all. So the trail does say who decided, but not in the field §3.7 names, and a consumer reading actors alone sees only the daemon. Exit condition: the resolved event's actor is the caller rather than the daemon, the event carries the mark, and a test fails when either half is dropped. |
| §5.4 | Entity ids are ULIDs stored as 26-char text | The ids this plugin MINTS are not. An event is `ev-<13-digit millisecond>-<8 hex digits>` and a parked action is `pk-` with the same shape (`internal/store/events.go`, `internal/store/parked.go`). The two properties §5.4 buys — unique and sortable by time — are both here, and the store is one JSON document in one operator's own state dir rather than a table anything else joins against, so a ULID dependency would buy nothing a millisecond and four random bytes do not. The 26-character ids this plugin HANDLES are the board's task ids, which arrive as ULIDs and are stored verbatim. |
