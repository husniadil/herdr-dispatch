...
// and performing the call are separate so that a test can read what a
// command line MEANS without a daemon to send it to.
type Call struct {
	Verb   verbs.Verb
...
func Root(run Runner) *cobra.Command {
	...
	root := &cobra.Command{
		Use:   "hdis",
		Short: "The dispatcher for the htask board",
		Long: "herdr-dispatch: it watches the htask board for ready work, brings a worker\n" +
			"pane up in Herdr for each ready task, delivers the task's goal, tracks the\n" +
			"worker, and stops at review, where the board's own review gate takes over.\n" +
	...
	root.PersistentFlags().StringVar(&g.project, "project", "", "The board to act on; defaults to every board (§4.2)")
	root.PersistentFlags().BoolVar(&g.allProjects, "all-projects", false, "Act across every board, which is this daemon's default")
	...
	// it. Added here so the command exists the moment the root does, and so
	// a test can ask for it without executing a call.
	root.InitDefaultCompletionCmd()
	return root
}

// newGroup is a parent command like `hdis parked`, which carries subcommands
// and does nothing itself.
//
// It has to be Runnable AND refuse arguments. Cobra returns help for a
// command that is not Runnable BEFORE it ever validates arguments, so NoArgs
// alone is unreachable on a bare parent: `hdis parked nonsense` would read as
// "no subcommand given", print help on stdout and exit 0, where §6.2 and §6.3
// promise one document and a failure status. RunE makes the parent Runnable
// so NoArgs is reached, and NoArgs turns the stray argument into the parse
// error main already renders as a USAGE envelope. Zero arguments is not a
// stray argument, so `hdis parked` still answers with its help. The sibling
// hit this first; the fix travels with the shape.
func newGroup(name string) *cobra.Command {
	return &cobra.Command{
		...
		// Being Runnable makes cobra add `hdis parked [flags]` to the usage
...
func buildVerb(v verbs.Verb, g *globals, run Runner) *cobra.Command {
	...
	// --follow is the CLI's own flag, not the registry's: it is a property
	// of the CONNECTION rather than an argument of the verb (§8.2 with
	// §7.1), and there is no tool call that could set it.
	var follow *bool
	if v.Name == "events" {
	...
	cmd.RunE = func(cmd *cobra.Command, argv []string) error {
		...
		// never typed is an argument the daemon would then have to tell
		// apart from one they did.
...
func (g *globals) request(v verbs.Verb, args map[string]any) (protocol.Request, error) {
	...
	return protocol.Request{
		...
		AllProjects: all,
		As:          as,
		// §3.6 with §3.7: a CLI invocation is one process per call, so this
		// argv IS the deliberate human act §3.7 asks a paneless `human` to
		// point at. That is what lets a paneless CLI call be the operator
		// where a paneless server door nobody declared is not. The pane
		// still wins (§3.2), and so does an explicit --as, so this changes
		// nothing for a call made from inside a pane.
		Operator: true,
	}, nil
}

// scope resolves --project and --all-projects into the pair the daemon reads.
//
// §4.4 has a list verb default to the resolved project; this daemon's default
// is every board, and that divergence is recorded in docs/contract-notes.md
// rather than left to be discovered. One daemon runs per user and drives
...
func Scope(project string, allProjects bool) (string, bool, error) {
	...
	// relative path is relative to the CALLER's working directory and the
...
// own, and codes.Of would call them UNAVAILABLE, which reads as "something
// was unreachable" for what is a typo. A refusal already named keeps its own,
