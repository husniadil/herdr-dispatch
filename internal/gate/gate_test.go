...
func TestUnconfiguredAllows(t *testing.T) {
	d := New(nil).Check(Request{Subject: "agent:wF:p1", Verb: "dispatch.dispatch", Target: "1"})
	if d.Decision != Allow {
...
func TestCommandSeesTheRequestOnStdin(t *testing.T) {
	...
	if d := g.Check(Request{Subject: "agent:wF:p1", Verb: "dispatch.stop", Target: "7"}); d.Decision != Allow {
	...
	for _, want := range []string{`"subject":"agent:wF:p1"`, `"verb":"dispatch.stop"`, `"target":"7"`} {
...
func TestDenyCarriesItsReason(t *testing.T) {
	...
	d := g.Check(Request{Verb: "dispatch.dispatch"})
...
func TestDeferIsItsOwnDecision(t *testing.T) {
	...
	if d := g.Check(Request{Verb: "dispatch.stop"}); d.Decision != Defer {
