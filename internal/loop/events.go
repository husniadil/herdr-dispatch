...
func (l *Loop) emitLocked(entity, kind, entityID, project string, detail map[string]any) store.Event {
	ev := store.Event{
		...
		// This plugin attributes nothing (§3.4): it derives no principal,
		// and every event here is its own daemon acting. So the actor is
		// the board principal this daemon writes with, which carries its
		// pane, and never a caller's.
		Actor:  l.principal(),
		Kind:   kind,
		Detail: detail,
	}
