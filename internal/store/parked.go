...
//
// It lives in the same document as the bindings for the same reason the
// reservations do — the document is written whole, so saving one can never
// drop another — and it is the one thing here that is NOT derivable from the
// board plus Herdr: a call that was refused before it reached either left no
// trace anywhere else.
type Parked struct {
	ID      string `json:"id"`
...
// still two. It is a name for one row in one operator's own file, which is
// all §9.3 asks a parked_id to be.
func NewParkedID(now time.Time) string {
	var b [4]byte
	if _, err := rand.Read(b[:]); err != nil {
		// A machine with no entropy still has a clock, and a duplicate id
		// is a worse answer than a less random one but a far better answer
		// than refusing to record the deferral at all.
		return fmt.Sprintf("pk-%013d-00000000", now.UnixMilli())
	}
	return fmt.Sprintf("pk-%013d-%s", now.UnixMilli(), hex.EncodeToString(b[:]))
}

// Waiting reports whether this action still wants the operator's attention.
// A failed action does: hiding it would make a verb that did not happen look
// like one that did.
